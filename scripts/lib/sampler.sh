...
sample_once() {
  ...
  for tenant in acme globex; do
    wk get pods -n "$tenant" -l 'job-role!=migration' -o jsonpath='{range .items[*]}{.metadata.labels.app\.kubernetes\.io/name}{"\t"}{.metadata.labels.version}{"\t"}{.metadata.labels.release-id}{"\t"}{range .status.conditions[?(@.type=="Ready")]}{.status}{end}{"\t"}{.status.initContainerStatuses[0].state}{"\n"}{end}' 2>/dev/null \
    ...
    wk get jobs -n "$tenant" -o jsonpath='{range .items[*]}{.metadata.name}{"\t"}{.metadata.labels.release-id}{"\t"}{.status.active}{"\t"}{.status.succeeded}{"\t"}{.status.failed}{"\n"}{end}' 2>/dev/null \
